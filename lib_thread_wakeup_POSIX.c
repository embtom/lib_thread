...
static struct signals_region *s_wakeup_signals = NULL;
...
int lib_thread__wakeup_init(void)
{
	...
	if (wakeup_signals->initialized == M_DEV_MAP_INITIALZED) {
		s_shm_fd = shm_fd;
		s_wakeup_signals = wakeup_signals;
		return EOK;
	}

	memset((void*)wakeup_signals,0,signals_region_size);
	pthread_mutex_init(&wakeup_signals->mmap_mtx, NULL );
	wakeup_signals->used_signals_max_cnt = signals_max_cnt;
	...
	wakeup_signals->initialized = M_DEV_MAP_INITIALZED;
	s_shm_fd = shm_fd;
	s_wakeup_signals = wakeup_signals;
...
int lib_thread__wakeup_create(wakeup_hdl_t *_wu_obj, unsigned _interval)
{
	...
	*_wu_obj = malloc(sizeof(**_wu_obj));
	...
	if(i >= s_wakeup_signals->used_signals_max_cnt) {
